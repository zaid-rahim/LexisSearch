...
# ─────────────────────────────────────────────────────────────────────────────
# 2. CSS — injected via st.html to guarantee <style> is parsed, not escaped
# ─────────────────────────────────────────────────────────────────────────────
# Streamlit sometimes escapes large <style> blocks inside st.markdown.
# Using multiple smaller injections + a direct HTML component is more reliable.
...
def load_documents():
    ...
    Uncomment whichever block matches your data format.
    """
    try:
        # CSV:
        ...
        return [
            f"[Placeholder #{i}] Replace load_documents() with your real corpus file path. "
            f"This is a placeholder for corpus row {i}."
            for i in range(n)
        ]
...
if len(documents) != doc_embeddings.shape[0]:
    st.warning(
        f"⚠️ Row count mismatch — {len(documents)} documents vs "
        f"{doc_embeddings.shape[0]} embeddings. Results may be misaligned."
    )
